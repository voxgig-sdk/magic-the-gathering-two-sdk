...
require("magic-the-gathering-two_types")
...
function MagicTheGatheringTwoSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function MagicTheGatheringTwoSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "MagicTheGatheringTwoSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function MagicTheGatheringTwoSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MagicTheGatheringTwoSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MagicTheGatheringTwoSDK: graphql: " .. msg
    res.graphql = errors
  end
