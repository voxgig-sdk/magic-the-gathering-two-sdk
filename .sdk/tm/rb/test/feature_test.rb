...
module MagicTheGatheringTwoFeatureHarness
  # True when this SDK was generated with the named feature.
  def self.has_feature?(name)
    f = MagicTheGatheringTwoConfig.shared_config["feature"]
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility.fetcher = server || MagicTheGatheringTwoFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless MagicTheGatheringTwoFeatureHarness.has_feature?(name)
        f = MagicTheGatheringTwoFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= MagicTheGatheringTwoFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(MagicTheGatheringTwoError)
        ...
        ctx.response = fetched.is_a?(Hash) ? MagicTheGatheringTwoResponse.new(fetched) : nil
    ...
    def populate_result(ctx, fetched, fetch_err)
      result = MagicTheGatheringTwoResult.new({})
      ctx.result = result
...
class FeatureTest < Minitest::Test
  H = MagicTheGatheringTwoFeatureHarness
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, MagicTheGatheringTwoError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, MagicTheGatheringTwoError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
