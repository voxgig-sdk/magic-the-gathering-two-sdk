...
// MagicTheGatheringTwo SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MagicTheGatheringTwoOmni::makeStructRunner(
    ...
    {
        return MagicTheGatheringTwoOmni::objmodel(MagicTheGatheringTwoOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return MagicTheGatheringTwoOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return MagicTheGatheringTwoOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MagicTheGatheringTwoOmni::structfix($now, Struct::undef()))
                        !== json_encode(MagicTheGatheringTwoOmni::structfix($val, Struct::undef()))) {
