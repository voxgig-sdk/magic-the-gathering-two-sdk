# magic-the-gathering-two-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MagicTheGatheringTwo SDK as
two agent tools — `magic-the-gathering-two_list` and `magic-the-gathering-two_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/magic-the-gathering-two-mcp)
make build

# 2. Provide credentials via the environment
export MAGIC_THE_GATHERING_TWO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user magic-the-gathering-two \
  -- /absolute/path/to/magic-the-gathering-two-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./magic-the-gathering-two-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// magic-the-gathering-two_list: first page of records
{ "entity": "card" }
{ "entity": "card", "query": { } }

// magic-the-gathering-two_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/magic-the-gathering-two-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user magic-the-gathering-two \
     -- "$PWD"/dist/*/magic-the-gathering-two-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `magic-the-gathering-two_list` and `magic-the-gathering-two_load` tools now appear
   in new sessions. Ask the agent to *"list card using magic-the-gathering-two"*
   and it calls `magic-the-gathering-two_list` with `{"entity":"card"}`.
...
```sh
export MAGIC_THE_GATHERING_TWO_APIKEY=sk_live_xxx            # API key
export MAGIC_THE_GATHERING_TWO_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `magic-the-gathering-two_list` tool
...
### Call the `magic-the-gathering-two_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `magic-the-gathering-two_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `magic-the-gathering-two_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MAGIC_THE_GATHERING_TWO_APIKEY` | API key sent with every request. |
| `MAGIC_THE_GATHERING_TWO_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

card | format | set | set_booster | subtype | supertype | type

### Smoke test via HTTP (raw JSON-RPC)

```sh
./magic-the-gathering-two-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"magic-the-gathering-two_load","arguments":{"entity":"card","query":{"id":1}}}}'
```
