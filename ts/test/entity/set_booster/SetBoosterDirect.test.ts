...
describe('SetBoosterDirect', async () => {
  ...
  test('direct-list-set_booster', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-set_booster', setup.live)) return
    if (skipIfMissingIds(t, setup, ["set_booster01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MAGIC_THE_GATHERING_TWO_TEST_SET_BOOSTER_ENTID']
