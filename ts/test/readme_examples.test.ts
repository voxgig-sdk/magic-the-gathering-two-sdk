...
//     (\`new MagicTheGatheringTwoSDK(...)\` / \`MagicTheGatheringTwoSDK.test(...)\` become
//     \`MagicTheGatheringTwoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MagicTheGatheringTwoSDK'
...
const TEST_SEED = {"entity":{"card":{"test01":{"id":"test01"}},"format":{"test01":{"id":"test01"}},"set":{"test01":{"id":"test01"}},"set_booster":{"test01":{"id":"test01"}},"subtype":{"test01":{"id":"test01"}},"supertype":{"test01":{"id":"test01"}},"type":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MagicTheGatheringTwoSDK(...)\` and any
// \`MagicTheGatheringTwoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MagicTheGatheringTwoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MagicTheGatheringTwoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
