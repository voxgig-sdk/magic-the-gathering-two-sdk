...
func TestSupertypeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MAGICTHEGATHERINGTWO_TEST_SUPERTYPE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func supertypeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MAGICTHEGATHERINGTWO_TEST_SUPERTYPE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAGICTHEGATHERINGTWO_TEST_SUPERTYPE_ENTID"])
