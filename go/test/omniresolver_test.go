...
import (
	...
	omni "github.com/voxgig-sdk/magic-the-gathering-two-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MagicTheGatheringTwoError); is {
				out := map[string]any{
					"name":    "MagicTheGatheringTwoError",
...
func omniCtx(arg any, client *sdk.MagicTheGatheringTwoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MagicTheGatheringTwoSDK); is {
			client = live
