...
func TestSetBoosterEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.SetBooster(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.SetBooster(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MAGIC_THE_GATHERING_TWO_TEST_SET_BOOSTER_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		setBoosterRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.set_booster")))
...
func set_boosterBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MAGIC_THE_GATHERING_TWO_TEST_SET_BOOSTER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MAGIC_THE_GATHERING_TWO_TEST_SET_BOOSTER_ENTID"])
