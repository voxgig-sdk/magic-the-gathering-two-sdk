# magic-the-gathering-two-cli

boru-driven command-line client **and** interactive REPL for the MagicTheGatheringTwo
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/magic-the-gathering-two-cli)
make build

# 2. See usage (words, entities, env vars)
./magic-the-gathering-two-cli --help

# 3. Provide credentials once, via the environment
export MAGIC_THE_GATHERING_TWO_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./magic-the-gathering-two-cli list card
./magic-the-gathering-two-cli load 1 card            # {id:1} shorthand
./magic-the-gathering-two-cli load '{id:1}' card       # explicit match map
./magic-the-gathering-two-cli list format

# 5. Override the API base URL for a single call
MAGIC_THE_GATHERING_TWO_BASE=https://api.example.com ./magic-the-gathering-two-cli list card

# 6. No arguments -> interactive REPL
./magic-the-gathering-two-cli
magic-the-gathering-two> list card
magic-the-gathering-two> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/magic-the-gathering-two-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/magic-the-gathering-two-cli list card
   ```
...
```sh
./magic-the-gathering-two-cli load 1 card          # scalar shorthand for {id:1}
./magic-the-gathering-two-cli load '{id:1}' card     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `magic-the-gathering-two>`). Each line is
evaluated as its own boru expression:

```text
$ ./magic-the-gathering-two-cli
magic-the-gathering-two> list card
magic-the-gathering-two> /help
magic-the-gathering-two> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/magic-the-gathering-two-cli`. |
...
(`./magic-the-gathering-two-cli <expr>`) and interactively (the REPL), and expressions compose the
