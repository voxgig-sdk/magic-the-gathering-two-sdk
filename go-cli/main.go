// Command MagicTheGatheringTwo-cli is a CLI + REPL for the MagicTheGatheringTwo SDK,
...
const prompt = "magic-the-gathering-two"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "card format set set_booster subtype supertype type"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MAGIC_THE_GATHERING_TWO_APIKEY (api key), MAGIC_THE_GATHERING_TWO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MAGIC_THE_GATHERING_TWO_APIKEY carries the API key and
	// MAGIC_THE_GATHERING_TWO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MAGIC_THE_GATHERING_TWO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MAGIC_THE_GATHERING_TWO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMagicTheGatheringTwoSDK(opts)
