...
// MagicTheGatheringTwo SDK retry feature
...
class MagicTheGatheringTwoRetryFeature extends MagicTheGatheringTwoBaseFeature
{
    ...
    public function init(MagicTheGatheringTwoContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MagicTheGatheringTwoContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MagicTheGatheringTwoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MagicTheGatheringTwoContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
