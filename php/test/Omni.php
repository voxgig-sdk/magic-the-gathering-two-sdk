...
//    only - a typed MagicTheGatheringTwoContext there would make every ctx
...
final class MagicTheGatheringTwoOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MagicTheGatheringTwoContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MagicTheGatheringTwoSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MagicTheGatheringTwoResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MagicTheGatheringTwoError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MagicTheGatheringTwoResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MagicTheGatheringTwoError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MagicTheGatheringTwoError(is_string($code) ? $code : '', $msg);
    }
