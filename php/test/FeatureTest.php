...
// MagicTheGatheringTwo SDK feature test
//
...
class FtCtrl extends MagicTheGatheringTwoControl
{
...
class FtClient
{
    ...
    private MagicTheGatheringTwoUtility $utility;

    public function __construct(MagicTheGatheringTwoUtility $utility, array $options)
    {
    ...
    public function get_utility(): MagicTheGatheringTwoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MagicTheGatheringTwoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MagicTheGatheringTwoContext $rootctx;

    // True when this SDK was generated with the named feature.
    public static function hasFeature(string $name): bool
    {
        if (self::$feature_config === null) {
            $config = MagicTheGatheringTwoConfig::shared_config();
            $f = $config['feature'] ?? [];
    ...
    public function __construct(
        ...
        $utility = new MagicTheGatheringTwoUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (MagicTheGatheringTwoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MagicTheGatheringTwoConfig::make_feature($name);
    ...
    private static function buildUrl(MagicTheGatheringTwoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MagicTheGatheringTwoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MagicTheGatheringTwoResponse($fetched);
        }
    ...
    private function fail(MagicTheGatheringTwoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MagicTheGatheringTwoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
