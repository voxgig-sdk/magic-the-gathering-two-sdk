...
from magicthegatheringtwo_sdk.magicthegatheringtwo_types import (
    SetBooster,
    SetBoosterListMatch,
)
...
class SetBoosterEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SetBoosterEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[SetBooster]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.SetBooster().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
