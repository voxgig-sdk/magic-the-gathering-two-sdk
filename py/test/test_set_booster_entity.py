...
class TestSetBoosterEntity:
    ...
    def test_should_stream(self):
        ...
        base = MagicTheGatheringTwoSDK.test(seed, None)
        seen = list(base.SetBooster(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = MagicTheGatheringTwoSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.SetBooster(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MAGIC_THE_GATHERING_TWO_TEST_SET_BOOSTER_ENTID JSON to run live")
        client = setup["client"]
...
def _set_booster_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MAGIC_THE_GATHERING_TWO_TEST_SET_BOOSTER_ENTID"))
    ...
    _live = env.get("MAGIC_THE_GATHERING_TWO_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("MAGIC_THE_GATHERING_TWO_TEST_EXPLAIN") == "TRUE",
        "live": _live,
