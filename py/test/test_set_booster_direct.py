...
class TestSetBoosterDirect:

    def test_should_direct_list_set_booster(self):
        setup = _set_booster_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-set_booster", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["set_booster01"]:
...
def _set_booster_direct_setup(mockres):
