# MagicTheGatheringTwo SDK test runner
...
class MagicTheGatheringTwoTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MagicTheGatheringTwoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MagicTheGatheringTwoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MagicTheGatheringTwoTestRunner.getenv("MAGIC_THE_GATHERING_TWO_TEST_LIVE")
        override = MagicTheGatheringTwoTestRunner.getenv("MAGIC_THE_GATHERING_TWO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MagicTheGatheringTwoTestRunner.getenv(key)
        ...
        explain = MagicTheGatheringTwoTestRunner.getenv("MAGIC_THE_GATHERING_TWO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MAGIC_THE_GATHERING_TWO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MagicTheGatheringTwoTestRunner._test_control is not None:
            return MagicTheGatheringTwoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MagicTheGatheringTwoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MagicTheGatheringTwoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MagicTheGatheringTwoTestRunner.load_test_control()
...
def load_env_local():
    MagicTheGatheringTwoTestRunner.load_env_local()


def env_override(m):
    return MagicTheGatheringTwoTestRunner.env_override(m)


def entity_data(v):
    return MagicTheGatheringTwoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MagicTheGatheringTwoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MagicTheGatheringTwoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MagicTheGatheringTwoTestRunner.load_test_control()


def live_client_options():
    return MagicTheGatheringTwoTestRunner.live_client_options()


def live_delay_ms():
    return MagicTheGatheringTwoTestRunner.live_delay_ms()
