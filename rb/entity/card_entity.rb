# MagicTheGatheringTwo SDK Card entity
...
class CardEntity
  ...
  # @return [Card, Hash] the loaded Card; raises MagicTheGatheringTwoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MagicTheGatheringTwoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Card>, Array] the matching Card items; raises MagicTheGatheringTwoError on failure
