...
class SetBoosterEntity
  ...
  def make
    opts = @_entopts.dup
    SetBoosterEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MagicTheGatheringTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [SetBooster, Hash] the current SetBooster data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MagicTheGatheringTwoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of SetBooster fields)
  def match_get
  ...
  #
  # @param reqmatch [SetBoosterListMatch, Hash, nil] match filter (any subset of
  #   SetBooster fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<SetBooster>, Array] the matching SetBooster items; raises MagicTheGatheringTwoError on failure
