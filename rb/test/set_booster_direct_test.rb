...
class SetBoosterDirectTest < Minitest::Test
  def test_direct_list_set_booster
    setup = set_booster_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-set_booster", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["set_booster01"].each do |_live_key|
...
def set_booster_direct_setup(mockres)
