...
class CardEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MAGIC_THE_GATHERING_TWO_TEST_CARD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def card_basic_setup(extra)
  ...
  entid_env_raw = ENV["MAGIC_THE_GATHERING_TWO_TEST_CARD_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "MAGIC_THE_GATHERING_TWO_TEST_CARD_ENTID" => idmap,
    "MAGIC_THE_GATHERING_TWO_TEST_LIVE" => "FALSE",
    "MAGIC_THE_GATHERING_TWO_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["MAGIC_THE_GATHERING_TWO_TEST_CARD_ENTID"])
