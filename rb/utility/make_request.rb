...
module MagicTheGatheringTwoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MagicTheGatheringTwoResponse.new({})
    ...
    if fetch_err
      ...
      response = MagicTheGatheringTwoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MagicTheGatheringTwoResponse.new(fetched)
    else
