...
require_relative 'MagicTheGatheringTwo_types'
...
class MagicTheGatheringTwoSDK
  ...
  def initialize(options = {})
    ...
    utility = MagicTheGatheringTwoUtility.new
    @_utility = utility
    ...
    feature_opts = MagicTheGatheringTwoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MagicTheGatheringTwoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MagicTheGatheringTwoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MagicTheGatheringTwoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MagicTheGatheringTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MagicTheGatheringTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MagicTheGatheringTwoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MagicTheGatheringTwoError.new(
        "#{op}_allow",
        "MagicTheGatheringTwoSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue MagicTheGatheringTwoError => err
    ...
    if fetched.is_a?(Hash)
      status = MagicTheGatheringTwoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MagicTheGatheringTwoError.new(
        "graphql_error", "MagicTheGatheringTwoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MagicTheGatheringTwoSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
